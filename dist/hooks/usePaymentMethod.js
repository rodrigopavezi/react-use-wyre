...
export default function usePaymentMethod() {
    const { wyre } = useWyre();
    const createPaymentMethod = useCallback(async ({ publicToken, country, paymentMethodType }) => {
    ...
    const attachBlockchain = useCallback(async ({ paymentMethodId, blockchains, notifyUrl, muteMessages }) => {
        const { data } = await wyre({
